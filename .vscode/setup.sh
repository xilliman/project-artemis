...
# DECKY_CLI_LOCATION="$(pwd)/cli/decky"
DECKY_CLI_LOCATION="/usr/local/bin/decky"

echo "$PNPM_LOCATION"
echo "$DOCKER_LOCATION"
echo "$DECKY_CLI_LOCATION"

echo "If you are using alpine linux, do not expect any support."

if [[ "$PNPM_LOCATION" =~ "which" ]]; then
...
# CLI_INSTALLED="/usr/local/bin/decky"
